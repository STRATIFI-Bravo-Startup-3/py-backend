...
class DepositSerializer(serializers.Serializer):
    ...
    def save(self):
        ...
        try:
            ...
            if not r.is_status != 200 or response.get("data", {}).get("reference", {}):
                raise ValidationError({"detail": "reference was not gotten"})
            ...
            raise ValidationError("Fail to connect to paystack")
...
class WalletTransactionSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if milestone_data:
            ...
            if milestone:
                milestone.title = milestone_data.get("title", milestone.title)
                milestone.description = milestone_data.get(
                    "description", milestone.description
                )
                milestone.due_date = milestone_data.get("due_date", milestone.due_date)
                milestone.save()
            else:
                Milestone.objects.create(wallet_transaction=instance, **milestone_data)
